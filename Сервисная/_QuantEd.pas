...
begin
  QOrders.DatabaseName := Main.GetCurrentBD;
  with BitBtn1 do
  begin
...
begin
  ...
  begin
    MessageDlg('Количество должно быть кратно указанному!', mtError, [mbOK], 0);
    QuantityEd.SetFocus;
    ModalResult := 0;
  end;
end;
...
begin
  ...
  begin
    ...
    aData.Host1 := Data.SysParamTable.FieldByName('TCPHostOpt').AsString; //'10.0.20.121';
    aData.Port1 := Data.SysParamTable.FieldByName('PortIN').AsInteger; //6003
    ...
    aData.aCodeBrandsStream := TStringStream.Create('');
