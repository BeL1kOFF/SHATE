...
begin
  lbLocalVersion.Visible := False;
  {$IFDEF LOCAL}
  lbLocalVersion.Visible := True;
  {$ENDIF}

  {$IFDEF NAVTEST}
  lbLocalVersion.Visible := True;
  lbLocalVersion.Caption := 'Внимание! тестовая версия';
  lbLocalVersion.Font.Color := clRed;
  lbLocalVersion.Left := 220;
  {$ENDIF}

  {$IFDEF PODOLSK}
  lbLocalVersion.Visible := True;
  lbLocalVersion.Caption := 'Склад Подольск (РФ)';
  {$ENDIF}
