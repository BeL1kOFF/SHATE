...
begin
  inherited;
    ...
    begin
      ...
      begin
        Mult := MemoryTable.FieldByName('Mult').AsInteger;
        if Mult = 0 then
          Mult := 1;

        if  ((MemoryTable.FieldByName('kol').AsFloat  >  MemoryTable.FieldByName('kolMax').AsFloat) or
                (MemoryTable.FieldByName('kol').AsFloat <= 0) or
                 (( MemoryTable.FieldByName('kol').AsInteger mod Mult) <> 0)) then
        begin
          rez:=1;
          break;
        end;
    ...
    if rez = 1 then
       MessageDlg('Введено неверное количество! Введите количество с учетом кратности и наличия товара.', mtError, [mbOk], 0)
    else
       ModalResult := MrOk;
end;
