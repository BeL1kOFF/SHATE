unit SMSAdapter.Logic.TSMSAdapterSender;
...
type
  TSMSAdapterSender = class(TInterfacedObject, ISenderAdapter)
  private
...
procedure TSMSAdapterSender.BeginSenderExecute;
type
...
begin
  ...
  try
    ...
    qrQuery.SQL.Text := 'SELECT smet.KeyField, smet.[Data BLOB] FROM [Shate-M$BLOB Export Table] smet WITH (UPDLOCK, ROWLOCK) WHERE smet.[Data sending] = 0 AND smet.[Error Count] < 5 AND smet.[Export Code] = ''TMS_DELIVERY_SMS''';
...
procedure TSMSAdapterSender.FinalizeAdapter;
begin
  FADOConection.Free();
  FCustomSenderAdapter := nil;
end;

procedure TSMSAdapterSender.InitAdapter(aCustomSenderAdapter: ICustomSenderAdapter);
begin
  ...
  FADOConection.ConnectionString := Format('Provider=SQLOLEDB.1;Integrated Security=SSPI;Persist Security Info=True;Initial Catalog=%s;Data Source=%s',
    [Loadxml('SMS.xml').Options.Db, Loadxml('SMS.xml').Options.Server]);
  FADOConection.CommandTimeout := 60000;
  FADOConection.LoginPrompt := False;
end;

procedure TSMSAdapterSender.LoadSenderAdapter;
begin
