...
History: PJN / 15-06-1998 1) Fixed the case where a single dot occurs on its own
         ...
         PJN / 11-09-1998 1) VC 5 project file is now provided
                          ...
                          4) All m_sLastCommandResponse.Format statements now display the value as returned from
                          GetLastError
                          ...
                          7) Extra m_sLastCommandResponse.Format statements have been added to help in debugging
...
BOOL CSMTPSocket::Connect(LPCTSTR pszHostAddress, int nPort, CString* sError)
{
  ...
  ASSERT(m_hSocket != INVALID_SOCKET);
	...
	return Connect((SOCKADDR*)&sockAddr, sizeof(sockAddr),sError);
}

BOOL CSMTPSocket::Connect(const SOCKADDR* lpSockAddr, int nSockAddrLen, CString* sError)
{
	...
	if(sError != NULL)
	{
		...
		sError->Format(_T("%d - %s"),Error,GetErrorText(&Error));
	}
	return FALSE;
}

BOOL CSMTPSocket::Send(LPCSTR pszBuf, int nBuf, CString* sError)
{
  //must have been created first
  ASSERT(m_hSocket != INVALID_SOCKET);
  //m_hSocket
  if(send(m_hSocket, pszBuf, nBuf, 0) != SOCKET_ERROR)
  {
	return TRUE;
  }
  if(sError != NULL)
	{
		DWORD Error;
		Error = WSAGetLastError();
		sError->Format(_T("%d - %s(%s)"),Error,GetErrorText(&Error),pszBuf);
	}
	return FALSE;
}
...
BOOL CSMTPAttachment::Attach(const CString& sFilename)
{
  ...
  {
	CString sError;
    sError.Format(_T("Failed to get the status for file %s, probably does not exist\n"), sFilename);
  ...
  {
	CString sError;
    sError.Format(_T("Failed to open file to be attached\n"));
	WriteToLogFile(sError);
    return FALSE;
  }
	...
	for (int pos=0, start=0; pos<=length; pos++)
	{
      ...
      CSMTPAddress To;
          ...
          SetLastError(ERROR_INVALID_DATA);
		    CString sError;
    sError.Format(_T("An error occurred while parsing the recipients string\n"));
	WriteToLogFile(sError);
...
BOOL CSMTPConnection::Connect(LPCTSTR pszHostName, int nPort)
{
  ...
  if (!m_SMTP.Create())
  {
    m_sLastCommandResponse.Format(_T("Failed to create client socket, GetLastError returns: %d\n"), GetLastError());
    return FALSE;
  }

  //Connect to the SMTP Host
  if (!m_SMTP.Connect(pszHostName, nPort,&m_sLastCommandResponse))
  {
    m_sLastCommandResponse.Format(_T("Could not connect to the SMTP server %s on port %d, GetLastError returns: %d\n"), pszHostName, nPort, GetLastError());
    return FALSE;
  }
  else
  {
    ...
    {
      m_sLastCommandResponse.Format(_T("An unexpected SMTP login response was received\n"));
      Disconnect();
      return FALSE;
    }
    ...
    if (!m_SMTP.Send(pszData, nCmdLength, &m_sLastCommandResponse))
    {
      Disconnect();
      m_sLastCommandResponse.Format(_T("An unexpected error occurred while sending the HELO command\n"));
      return FALSE;
    }
    ...
    {
      Disconnect();
      m_sLastCommandResponse.Format(_T("An unexpected HELO response was received\n"));
      return FALSE;
    } 
...
BOOL CSMTPConnection::Disconnect()
{
  ...
  if (m_bConnected)
  {
    ...
    if (!m_SMTP.Send(sBuf, nCmdLength, &m_sLastCommandResponse))
      m_sLastCommandResponse.Format(_T("Failed in call to send QUIT command, GetLastError returns: %d\n"), GetLastError());
    ...
    if (!bSuccess)
    {
      ...
      m_sLastCommandResponse.Format(_T("An unexpected QUIT response was received\n"));
    }
    ...
    m_sLastCommandResponse.Format(_T("Already disconnected from SMTP server, doing nothing\n"));
...
BOOL CSMTPConnection::SendMessage(CSMTPMessage& Message)
{
  ...
  if (!m_SMTP.Send(pszMailFrom, nCmdLength,&m_sLastCommandResponse))
  {
	m_sLastCommandResponse.Format(_T("Failed in call to send MAIL command, GetLastError returns: %d\n"), GetLastError());
    return FALSE;
  }
  ...
  {
    SetLastError(ERROR_BAD_COMMAND);
    m_sLastCommandResponse.Format(_T("An unexpected MAIL response was received\n"));
    return FALSE;
  } 
  ...
  if (!m_SMTP.Send(pszDataCommand, nCmdLength, &m_sLastCommandResponse))
  {
    m_sLastCommandResponse.Format(_T("Failed in call to send MAIL command, GetLastError returns: %d\n"), GetLastError());
    return FALSE;
  }
  ...
  {
    SetLastError(ERROR_BAD_COMMAND);
    m_sLastCommandResponse.Format(_T("An unexpected DATA response was received\n"));
    return FALSE;
  } 
  ...
  if (!m_SMTP.Send(pszHeader, nCmdLength, &m_sLastCommandResponse))
  {
    m_sLastCommandResponse.Format(_T("Failed in call to send the header, GetLastError returns: %d\n"), GetLastError());
    return FALSE;
  }
  ...
  if (Message.m_Attachments.GetSize())
  {
    ...
    if (!m_SMTP.Send(psBodyHeader, nCmdLength, &m_sLastCommandResponse))
    {
      m_sLastCommandResponse.Format(_T("Failed in call to send the body header, GetLastError returns: %d\n"), GetLastError());
      return FALSE;
    }
  }
  ...
  if (!m_SMTP.Send(pszBody, nCmdLength, &m_sLastCommandResponse))
  {
    m_sLastCommandResponse.Format(_T("Failed in call to send the header, GetLastError returns: %d\n"), GetLastError());
    return FALSE;
  }
  ...
  for (i=0; i<Message.m_Attachments.GetSize(); i++)
  {
    CSMTPAttachment* pAttachment = Message.m_Attachments.GetAt(i);
		...
		if (!m_SMTP.Send(pszContent, nCmdLength, &m_sLastCommandResponse))
		{
			m_sLastCommandResponse.Format(_T("Failed in call to send Mime attachment header, GetLastError returns: %d\n"), GetLastError());
			return FALSE;
		}

    //Then send the encoded attachment
    if (!m_SMTP.Send(pAttachment->GetEncodedBuffer(), pAttachment->GetEncodedSize(),&m_sLastCommandResponse))
    {
      m_sLastCommandResponse.Format(_T("Failed in call to send the attachment, GetLastError returns: %d\n"), GetLastError());
      return FALSE;
    }
  }
  ...
  if (Message.m_Attachments.GetSize())
  {
	  ...
	  if (!m_SMTP.Send(pszFinalBoundary, nCmdLength, &m_sLastCommandResponse))
	  {
		  m_sLastCommandResponse.Format(_T("Failed in call to send Mime attachment header, GetLastError returns: %d\n"), GetLastError());
		  return FALSE;
	  }
  ...
  if (!m_SMTP.Send(pszEOM, nCmdLength,&m_sLastCommandResponse))
  {
      m_sLastCommandResponse.Format(_T("Failed in call to send end of message indicator, GetLastError returns: %d\n"), GetLastError());
      return FALSE;
  }
  ...
  {
    SetLastError(ERROR_BAD_COMMAND);
    m_sLastCommandResponse = m_sLastCommandResponse+ _T("\n \tAn unexpected end of message response was received\n");
    return FALSE;
  } 
...
BOOL CSMTPConnection::SendRCPTForRecipient(CSMTPAddress& recipient)
{
  ...
  LPTSTR pszRCPT = sBuf.GetBuffer();

  int nCmdLength = strlen(T2A(pszRCPT));
  if (!m_SMTP.Send(T2A(pszRCPT), nCmdLength, &m_sLastCommandResponse))
  {
    m_sLastCommandResponse.Format(_T("Failed in call to send RCPT command, GetLastError returns: %d\n"), GetLastError());
    return FALSE;
  }
  ...
  if (!ReadCommandResponse(250))
  {
    ...
    SetLastError(ERROR_BAD_COMMAND);
    m_sLastCommandResponse.Format(_T("An unexpected RCPT response was received\n"));
    return FALSE;
  } 
...
BOOL CSMTPConnection::SendAutorization(CStringA sLogin, CStringA sPass)
{
	USES_CONVERSION;	
	...
	if (!m_SMTP.Send(pszMailFrom, nCmdLength, &m_sLastCommandResponse))
	{
		...
		m_sLastCommandResponse.Format(_T("Failed in call to send AUTH LOGIN command, GetLastError returns: %d\n"), GetLastError());
		return FALSE;
	}
	...
	char* m_pszEncoded;
	CSMTPAttachment code;
	m_pszEncoded = NULL;
	int nOutSize;
	
	m_pszEncoded = code.CodeBase64(sLogin,&nOutSize);
	if (!m_SMTP.Send(m_pszEncoded, nOutSize, &m_sLastCommandResponse))
	{
		delete(m_pszEncoded);
		m_sLastCommandResponse.Format(_T("Failed in call to send AUTH LOGIN command, GetLastError returns: %d\n"), GetLastError());
		return FALSE;
	...
	m_pszEncoded = NULL;
	m_pszEncoded = code.CodeBase64(sPass,&nOutSize);
	if (!m_SMTP.Send(m_pszEncoded, nOutSize, &m_sLastCommandResponse))
	{
		delete(m_pszEncoded);
		m_sLastCommandResponse.Format(_T("Failed in call to send AUTH LOGIN command, GetLastError returns: %d\n"), GetLastError());
...
char* CSMTPAttachment::CodeBase64(CStringA sValue, int *iRet)
{
	char * chRet;
	chRet = NULL;
	if(chRet != NULL)
	...
	*iRet = -1;
	...
	char* pszIn = new char[sValue.GetLength()];
	pszIn = sValue.GetBuffer(sValue.GetLength());
	
	//allocate the encoded buffer
	int nOutSize = Base64BufferSize(sValue.GetLength());
	chRet = new char[nOutSize];
	//Do the encoding
	EncodeBase64(pszIn, sValue.GetLength(), chRet, nOutSize, iRet);
	return chRet;
}