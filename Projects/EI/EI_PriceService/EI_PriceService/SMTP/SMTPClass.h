...
//methods
  BOOL  Create();
  BOOL  Connect(LPCTSTR pszHostAddress, int nPort = 110,CString* sError=NULL);
  BOOL  Send(LPCSTR pszBuf, int nBuf, CString* sError);
  void  Close();
  int   Receive(LPSTR pszBuf, int nBuf);
  BOOL  IsReadible(BOOL& bReadible);

protected:
  BOOL   Connect(const SOCKADDR* lpSockAddr, int nSockAddrLen, CString* sError=NULL);
  SOCKET m_hSocket;
};
...
//methods
  ...
  char * CodeBase64(CStringA sValue, int* iRet);
...
//Methods
  ...
  void    SetTimeout(DWORD dwTimeout) { m_dwTimeout = dwTimeout; };
	...
	BOOL    SendAutorization(CStringA sLogin, CStringA sPass);
protected:
