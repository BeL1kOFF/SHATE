...
{    
	...
	SendMessageToParent(pgvItem->row, pgvItem->col, GVN_APPLYFILTER);
...
void CGridCtrl::OnSelecting(const CCellID& currentCell)
{
    ...
    switch(m_MouseMode)
    {
        case MOUSE_SELECT_ALL: SelectRows(currentCell);   break;
        case MOUSE_SELECT_COL: SelectRows(currentCell);   break;
...
BOOL CGridCtrl::SetColumnCount(int nCols)
{
    ...
    // Change the number of columns.40329178
                            ...
                            int nColumn  /* = -1 */)
{
    ...
    else
    {
		int iStart;
		iStart = 0;
        m_arColWidths.InsertAt(nColumn, iStart);
        for (int row = 0; row < m_nRows; row++) 
        {
            GRID_ROW* pRow = m_RowData[row];
            if (!pRow) return -1;
...
int CGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */)
{
    ...
    {
		int iStart = 0;
        m_arRowHeights.InsertAt(nRow, iStart);
