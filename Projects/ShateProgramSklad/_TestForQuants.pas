...
begin
   ...
   begin
      ...
      begin
          ...
          begin
            AnalogOrderDet.First;
            while not AnalogOrderDet.Eof do
            begin
                if (AnalogOrderDet.FieldByName('Quant').AsString <> '')and(AnalogOrderDet.FieldByName('Quant').AsString <> '0') then
                begin
                  ...
                  GridAnalog.Cells[2,GridAnalog.RowCount-1]:= AnalogOrderDet.FieldByName('An_Code').AsString;
                  GridAnalog.Cells[3,GridAnalog.RowCount-1]:= AnalogOrderDet.FieldByName('An_Brand').AsString;
                  GridAnalog.Cells[5,GridAnalog.RowCount-1]:= OrderDetTable.FieldByName('Quantity').AsString;
                  GridAnalog.Cells[10, GridAnalog.RowCount-1] := AnalogOrderDet.FieldByName('Descr').AsString;

                  if Data.Curr = 2 then
                    if AnalogOrderDet.FieldByName('SaleQ').AsString = '1' then
                      begin
                         GridAnalog.Cells[4,GridAnalog.RowCount-1]:= FloatToStr(XRound(Data.Rate * AnalogOrderDet.FieldByName('Price').AsCurrency,-1));
                      end
                     else
                      begin
                         GridAnalog.Cells[4,GridAnalog.RowCount-1]:= FloatToStr(XRound(Data.Rate *(Data.GetDiscount(0,0,AnalogOrderDet.FieldByName('Brand_id').AsInteger)) * AnalogOrderDet.FieldByName('Price').AsCurrency,-1));
                      end
                  else
                     if AnalogOrderDet.FieldByName('SaleQ').AsString = '1' then
                        begin
                          GridAnalog.Cells[4,GridAnalog.RowCount-1]:= FloatToStr(XRound(Data.Rate * AnalogOrderDet.FieldByName('Price').AsCurrency,2));
                        end
                     else
                        begin
                           GridAnalog.Cells[4,GridAnalog.RowCount-1]:= FloatToStr(XRound(Data.Rate *(Data.GetDiscount(0,0,AnalogOrderDet.FieldByName('Brand_id').AsInteger)) * AnalogOrderDet.FieldByName('Price').AsCurrency,2));
                        end;
                   ...
                   if AnalogOrderDet.FieldByName('SaleQ').AsString = '1' then
                     begin
                        NewElement.rEuro := XRound(AnalogOrderDet.FieldByName('Price').AsCurrency,2);
                     end
                   else
                     begin
                        NewElement.rEuro := XRound((Data.GetDiscount(0,0,AnalogOrderDet.FieldByName('Brand_id').AsInteger)) * AnalogOrderDet.FieldByName('Price').AsCurrency,2);
                     end;
