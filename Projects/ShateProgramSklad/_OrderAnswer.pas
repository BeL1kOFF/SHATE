...
type
  ...
  public
    procedure Init(aRespondList: TStringList; aCurrencyMain, aCurrencyDop, aLotusNumber: string);
...
var
    ...
    Worksheet, Workbook, CountSheet: Variant;
    Iter, iRow : integer;
    aRange: Variant;
...
procedure TOrderAnswer.Button3Click(Sender: TObject);
begin
     ColorDialog.Execute();
     MessageDlg(inttostr(ColorDialog.Color),mtInformation,[MBOK],0);
end;
...
//            GridAnalog.Cells[2,GridAnalog.RowCount-1]:= ;
    ...
    iWidth := Round((GridAnalog.Width - GridAnalog.Left-13)/(GridAnalog.ColCount));
    ...
    GridAnalog.ColWidths[6]:= GridAnalog.Width- GridAnalog.Left - (GridAnalog.ColCount-1)*iWidth-13;
...
begin
  ...
  if Data.BrandTable.Locate('Description', s2, []) and
     Data.XCatTable.FindKey([s1]) then
  Result := Data.XCatTable.FieldByName('Name_Descr').Value;
  //Trim(Data.XCatTable.FieldByName('Name').AsString + ' ' + Data.XCatTable.FieldByname('Description').AsString);
end;
...
//         if NewElement.State = -1 then
  ...
  {       else
            ImageListCheck.GetIcon(0,IconNew);}
...
procedure TOrderAnswer.Init(aRespondList: TStringList; aCurrencyMain,
  aCurrencyDop, aLotusNumber: string);
begin
