...
const
  ...
  cWebUpdateTypeDescr: array[TUpdatePackageType] of string = (
    ...
    'Пакет обновления - остатки и цены (рынки)',
...
type
  ...
  private
    ...
    function DoInstall_QuantsStock(const aQuantsNum: string): Boolean;
    function DoInstall_QuantsStockNew(): Boolean;
...
//    function DiskUpdateExecute: Integer;
    ...
    //function DoUpdateQuants(const aPath: string): Boolean;
    function DoUpdateQuants2(const aPath: string): Boolean;
    function DoUpdateQuantsNew(const aPath: string): Boolean;

    function DoUpdateStockQuantsNew(const aPath: string): Boolean;
    function DoUpdateQuantsStock(const aPath: string; const aQuantNum: string): Boolean;
  ...
  private
    ...
    fPackageTypeCode: string;    //код типа обновления (data, data_d, news, quants, quantsXX, picts_d)
...
begin
  ...
  //aCode = quants2, quants3 .. quants16
  if SameText(Copy(aCode, 1, 6), 'quants') then
  begin
...
var
  ...
  aValue, aType, anArt: Integer;
  aFirst: Boolean;
  aBufPos, aBufSize: Integer;
begin
...
begin
  ...
  {sMessage:= 'Загрузка применяемости в каталог...';
  Synchronize(ShowProgressBase);
  ...
  begin
    ...
    begin
      s := '';
      if (FieldByName('Tecdoc_id').AsInteger <> 0)and( FieldByName('Primen').AsString = '') then
      begin
        with Data.LoadArtTypTable do
        begin
          SetRange([tbl.FieldByName('Tecdoc_id').AsInteger],
                   [tbl.FieldByName('Tecdoc_id').AsInteger]);
          First;
          while not Eof do
          begin
            if Pos(',' + FieldByName('Typ_id').AsString + ',', ',' + s + ',') = 0 then
            begin
              if s <> '' then
                s := s + ',';
              s := s + FieldByName('Typ_id').AsString;
            end;
            Next;
          end;
          CancelRange;
...
//  Data.RemoveTableFromBase(Data.NomList.TableName + '_New');
...
//не импортируем системную таблицу - она другая в складской версии
{
  fname := Path + 'sys.csv';
  UpdateTable := TDBISAMTable.Create(nil);
  try
...
//применяемость-----------------------------------------------------------------
{
  UpdateTable := TDBISAMTable.Create(nil);
  try
    makeTableForUpdate(UpdateTable, Data.TableCarFilter);

    fname := Path + 'car.csv';
    if FileExists(fname) then
    begin
      ImportTableBase(UpdateTable, fname, 'Загрузка авто...', 'TYPE_ID');
      LoadPrimenMemo(UpdateTable);
    end;
    ...
    CopyFile(PCHAR(fTmpPath + 'UpdateInfo.csv'), PCHAR(Data.Import_Path + 'UpdateInfo.csv'), False);
{
  try //except
    ...
    try //finally
      Readln(f, aLine);
      fNewDataVersion := aLine;
      Readln(f, aLine);
      fNewDescretVersion := aLine;
    finally
      CloseFile(f);
    end;
...
begin
//  Result := DoUpdateQuants2(fTmpPath); {без [002_]}
    Result := DoUpdateQuantsNew(fTmpPath);
end;

function TUpdateDataThrd.DoInstall_QuantsStock(const aQuantsNum: string): Boolean;
begin
  if SameText(Copy(aQuantsNum, 1, 6), 'quants') then
    Result := DoUpdateQuantsStock(fTmpPath, Copy(aQuantsNum, 7, MaxInt))
  else
    raiseUpdateErr('Неизвестная база остатков: "' + aQuantsNum + '"');
end;

function TUpdateDataThrd.DoInstall_QuantsStockNew: Boolean;
begin
  Result := DoUpdateStockQuantsNew(fTmpPath);
end;
...
var
  ...
  sGroup_id, sGroup_descr, sSubgroup_id, sSubgroup_descr, sDescr: string;
  sCode, sCode2, sName, sDescription: string;
  sT1, sT2, sTecdoc_id, sNew, sSale: string;
  sMult, sUsa, sPrice, sTitle: string;
  sCat_id, sAn_code, sAn_brand, sAn_id,  sAnShortCode, sGen_Id, sLocked, sShortOE: string;
  sAction: string;
  ...
  sChild_Code, sChild_Brand, sChild_id, sQuantity: string;
     ...
     function MakeDeleting(Field4Delete, TableName,FileName, Msg: string ): boolean;
   begin
  ...
  function MakeAddingAnalog(TableName,FileName, Msg: string ): boolean;
  begin
    if (_Data.GetFileSize_Internal(Path + FileName) > 0) then
    begin
      fProgressMessage:= Msg;
...

{
    ...
    Synchronize(CurrProgressBase);
    CopyTableBase('032', '032_New');

    UpdateTable := TDBISAMTable.Create(nil);
    try
      UpdateTable.TableName := Data.TableCarFilter.TableName + '_New';
      UpdateTable.DatabaseName := fLocalDatabase.DatabaseName;
      LoadPrimenMemo(UpdateTable);
      UpdateTable.Close;
    finally
...
// измененные --------------------------------------------------
   ...
   { if (_Data.GetFileSize_Internal(Path + 'ana_0.csv') > 0) or
       (_Data.GetFileSize_Internal(Path + 'ana_1.csv') > 0) or
       (_Data.GetFileSize_Internal(Path + 'ana_2.csv') > 0) then
    begin
      fProgressMessage := 'Копирование аналогов...';
      Synchronize(CurrProgressBase);
      CopyTableBase('007', '007_New');
    end;
    ...
    begin
      ...
      begin
        ...
        if (anIDs.Count >= cDeleteBatchCount) or (aReader.Eof) then
        begin
          ExecQuery('DELETE from [007_New] WHERE ID IN (' + anIDs.CommaText + ')', TestQuery);
          anIDs.Clear;
        end;
    ...
    // Аналоги (измененные) --------------------------------------------------
    if (_Data.GetFileSize_Internal(Path + 'ana_2.csv') > 0) then
    begin
      fProgressMessage:= 'Обновление аналогов...';
      Synchronize(ShowProgressBase);
      UpdateTable := makeSafeTable('007_new');
      try
        UpdateTable.Open;

        aReader.Open(Path + 'ana_2.csv');
        while not aReader.Eof do
        begin
          if Terminated then //check terminated
            Abort;
    ...
    // Коды ОЕ ---------------------------------------------------------------
    if (_Data.GetFileSize_Internal(Path + 'oe_0.csv') > 0) or
       (_Data.GetFileSize_Internal(Path + 'oe_1.csv') > 0) or
       (_Data.GetFileSize_Internal(Path + 'oe_1_fix.csv') > 0) then
    begin
      fProgressMessage:= 'Копирование ОЕ...';
      Synchronize(CurrProgressBase);
      CopyTableBase('016', '016_New');
    end;

    // Коды ОЕ (удаленные) ---------------------------------------------------
    if (_Data.GetFileSize_Internal(Path + 'oe_0.csv') > 0) then
    begin
      fProgressMessage:= 'Удаление ОЕ...';
      Synchronize(ShowProgressBase);
      ...
      begin
        ...
        if (anIDs.Count >= cDeleteBatchCount) or (aReader.Eof) then
        begin
          ExecQuery('DELETE from [016_New] WHERE ID IN (' + anIDs.CommaText + ')', TestQuery);
          anIDs.Clear;
        end;
    ...
    if (_Data.GetFileSize_Internal(Path + 'oe_1.csv') > 0) then
    begin
      fProgressMessage:= 'Загрузка ОЕ...';
      Synchronize(ShowProgressBase);
      UpdateTable := makeSafeTable('016_new');
      try
        UpdateTable.Open;
        ...
        begin
          ...
          sAn_code := aReader.Fields[3];
          sAn_brand := aReader.Fields[4];
          sShortOE := Data.CreateShortCode(sAn_code);
          if sShortOE <> '' then
            aSimbOE := Ord( sShortOE[1] )
          else
            aSimbOE := Ord( '?' );
          ...
          begin
            ...
            UpdateTable.FieldByName('ID').AsString := sID;
            UpdateTable.FieldByName('Cat_id').AsString := sCat_id;
            UpdateTable.FieldByName('Code').AsString := sAn_code;
            UpdateTable.FieldByName('Code2').AsString := sAn_brand;
            UpdateTable.FieldByName('ShortOE').AsString := sShortOE;
            UpdateTable.FieldByName('SIMB').AsInteger := aSimbOE;
            UpdateTable.Post;
          end;
...
// !!! FIX !!! Коды ОЕ (новые) -------------------------------------------------------
// то что не проимпортилось прошлым обновлением - добавляем с проверкой на существование ID
    if (_Data.GetFileSize_Internal(Path + 'oe_1_fix.csv') > 0) then
    begin
      fProgressMessage:= 'Загрузка ОЕ(fix)...';
      Synchronize(ShowProgressBase);
      ...
      try
        ...
        aReader.Open(Path + 'oe_1_fix.csv');
        while not aReader.Eof do
        begin
          if Terminated then //check terminated
            Abort;
...
function TUpdateDataThrd.DoUpdateQuants2(const aPath: string): Boolean;
var
  sNewIDs: string;

  procedure DoUpdate(aNomTable, aNewQuantTable, aUpdateTable: TDBISamTable);
  var
    aPer: Integer;
  begin
    with aNewQuantTable do
    begin
      IndexName := 'Code';
      aNewQuantTable.Open;
      First;
      
      aNomTable.IndexName := 'Code2';
      aNomTable.Open;
      aNomTable.First;

      while aNomTable.FieldByName('Code2').AsString = '' do
      begin
        aNomTable.Next;
        if aNomTable.Eof then
          Break;
      end;

      aUpdateTable.Open;
      while not aNomTable.EOF do
      begin
        CallProgress( GetPercent(aNomTable.RecNo, aNomTable.RecordCount) );

        //Main.sIDs
        if aNomTable.FieldByName('Code2').AsString = FieldByName('sCode').AsString then
        begin
          while aNomTable.FieldByName('Brand_id').AsInteger > Data.UpdateQuant2.FieldByName('Brand_id').AsInteger do
          begin
            Data.UpdateQuant2.Next;
            if Data.UpdateQuant2.Eof then
              Break;
            if aNomTable.FieldByName('Code2').AsString <> FieldByName('sCode').AsString then
              Break;
          end;
          ...
          if aNomTable.FieldByName('Code2').AsString = FieldByName('sCode').AsString then
          begin
            if aNomTable.FieldByName('Brand_id').AsInteger = FieldByName('Brand_id').AsInteger then
            begin
              aUpdateTable.Append;
              aUpdateTable.FieldByName('Cat_id').Value   := aNomTable.FieldByName('Cat_id').AsInteger;
              aUpdateTable.FieldByName('Quantity').Value := FieldByName('Quants').AsString;
              aUpdateTable.FieldByName('Latest').AsInteger := 0;
              if (FieldByName('Quants').AsString <> '') and (FieldByName('Quants').AsString <> '0') then
              begin
                if Data.memQuants.FindKey([aUpdateTable.FieldByName('Cat_id').AsInteger]) then
                begin
                  //раньше остатки эти были нулевые
                  aUpdateTable.FieldByName('Latest').AsInteger := 1;

                  if sNewIDs <> '' then
                    sNewIDs := sNewIDs + ', ' + aUpdateTable.FieldByName('Cat_id').AsString
                  else
                    sNewIDs := aUpdateTable.FieldByName('Cat_id').AsString;
                end;

                if Main.sIDs <> '' then
                  Main.sIDs := Main.sIDs + ',' + aUpdateTable.FieldByName('Cat_id').AsString
                else
                  Main.sIDs := aUpdateTable.FieldByName('Cat_id').AsString;
              end;

              if Trim(FieldByName('PRICE').AsString) <> '' then
              begin
                aUpdateTable.FieldByName('Price').Value := Main.AToCurr(FieldByName('PRICE').AsString);
              end
              else
                aUpdateTable.FieldByName('Price').Value := Main.AToCurr('0');
              if FieldByName('SALE').AsString = '1' then
                aUpdateTable.FieldByName('Sale').Value := '1'
              else
                aUpdateTable.FieldByName('Sale').Value := '0';

              aUpdateTable.FieldByName('QuantNew').AsString := FieldByName('QuantNew').AsString;
              aUpdateTable.Post;
          ...
          begin
            ...
            aNomTable.Next;
      end; //while not aNomTable.EOF

      aNomTable.Close;
    end;
    aNewQuantTable.Close;
//!!!!!!!!! для теста только новых остатков - откомментить
//    Main.sIDs := sNewIDs;
    Main.sLatestIDs := sNewIDs;
  end;
...
begin
  ...
  TestTable(Data.UpdateQuant2, Data.data_psw);
  ...
  TestTable(Data.memQuants, Data.data_psw);
  Data.memQuants.EmptyTable;
  ExecQuerySafe(
    ' INSERT INTO "MEMORY\memQuants" ' +
    ' SELECT DISTINCT CAT_ID FROM [012] WHERE quantity = ''0'' or quantity = '''' '
  );
      ...
      try
        ...
        begin
          ...
          begin
            Append;
              FieldByName('sCode').AsString := cat_code;
              FieldByName('Brand_id').AsInteger := Data.memBrand.FieldByName('Brand_id').AsInteger;
              FieldByName('Quants').AsString := aReader.Fields[1];
              FieldByName('PRICE').AsString := '33,78';//aReader.Fields[2];
              FieldByName('SALE').AsString := aReader.Fields[3];
              FieldByName('QuantNew').AsString := aReader.Fields[5];
            Post;
          end;
...
//******************************************************************************
    ...
    Synchronize(ShowProgressBase);
    DoUpdate(Data.Nom, Data.UpdateQuant2, UpdateTable);
    ...
    Data.UpdateQuant2.Close;
    Data.UpdateQuant2.EmptyTable;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        aReader.ReturnLine;
        rate := Main.AToFloat(aReader.Fields[1]);
        if rate > 0 then
          FieldByName('Eur_usd_rate').Value := rate;

        aReader.ReturnLine;
        rate := Main.AToFloat(aReader.Fields[1]);
        if rate > 0 then
          FieldByName('Eur_rate').Value := rate;

        aReader.ReturnLine;
        rate := Main.AToFloat(aReader.Fields[1]);
        if (rate > 0) and (aReader.Fields[1] <> '') then
          FieldByName('Eur_RUB_rate').Value := rate;
...
function TUpdateDataThrd.DoUpdateQuantsStock(const aPath,
  aQuantNum: string): Boolean;
var
  sNewIDs: string;
  sQuantsIDs: string;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              aUpdateTable.FieldByName('Cat_id').Value   := aNomTable.FieldByName('Cat_id').AsInteger;
              aUpdateTable.FieldByName('Quantity').Value := FieldByName('Quants').AsString;
              aUpdateTable.FieldByName('Limit').AsInteger := FieldByName('Limit').AsInteger;
              aUpdateTable.FieldByName('Price').AsFloat := Main.AToCurr(FieldByName('Price').AsString);

              //!!! сделать для каждой таблицы остатокв
              //aUpdateTable.FieldByName('Latest').AsInteger := 0;
              if (FieldByName('Quants').AsString <> '') and (FieldByName('Quants').AsString <> '0') then
              begin
                if Data.memQuants.FindKey([aUpdateTable.FieldByName('Cat_id').AsInteger]) then
                begin
                  //раньше эти остатки были нулевые
                  //!!! сделать для каждой таблицы остатокв
                  //aUpdateTable.FieldByName('Latest').AsInteger := 1;
                ...
                if sQuantsIDs <> '' then
                  sQuantsIDs := sQuantsIDs + ',' + aUpdateTable.FieldByName('Cat_id').AsString
                else
                  sQuantsIDs := aUpdateTable.FieldByName('Cat_id').AsString;
              end;

              { //для складскиз не надо
              if Trim(FieldByName('PRICE').AsString) <> '' then
              begin
                aUpdateTable.FieldByName('Price').Value := Main.AToCurr(FieldByName('PRICE').AsString);
              end
...
//!!! сделать для каждой таблицы остатокв
//    Main.sLatestIDs := sNewIDs;
//    Main.sIDs := sQuantsIDs;
  end;

var
  UpdateTable, aDestTable: TDBISAMTable;
  cb, cat_code, cat_brand: string;
  p: integer;
  aReader: TCSVReader;
  aQuerySel: TDBISAMQuery;
begin
  ...
  //находим нужную таблицу остатков
  aDestTable := TDBISamTable(Data.FindComponent('QuantTable' + aQuantNum));
  if aDestTable = nil then
    raiseUpdateErr('База остатков №' + aQuantNum + ' не найдена');
  ...
  Main.sIDs := '';
  Data.RemoveTableFromBase(aDestTable.TableName + '_New');
  ...
  try //finally
    UpdateTable.DatabaseName := Data.Database.DatabaseName;
    UpdateTable.TableName := aDestTable.TableName + '_New';
    Data.CopyMaketTable(UpdateTable, aDestTable);
    TestTable(UpdateTable);
        ...
        begin
          ...
          begin
            Append;
              ...
              FieldByName('Brand_id').AsInteger := Data.memBrand.FieldByName('Brand_id').AsInteger;
              FieldByName('Quants').AsString := aReader.Fields[1];
              FieldByName('Limit').AsString := aReader.Fields[4];
              FieldByName('Price').AsString := aReader.Fields[2];
            Post;
          end;
...
//******************************************************************************
    if not fNomListCreated or not Data.Nom.Exists then //пересоздать NomList
    begin
      fProgressMessage := 'Загрузка номенклатуры...';
      Synchronize(ShowProgressBase);
...
//******************************************************************************
    ...
    Data.Nom.Close;
    //Data.Nom.EmptyTable; //оставляем жить для остальных пакетов с остатками
...
//!!! сделать для каждой таблицы остатокв
  ...
  if Main.sLatestIDs <> '' then
    Main.sLatestIDs := '(Cat_id IN ('+Main.sLatestIDs+'))';  //видимо, обновленные ID'шки
}

  Result := True;
end;

function TUpdateDataThrd.DoUpdateStockQuantsNew(const aPath: string): Boolean;
var
  UpdateTable: TDBISAMTable;
  cb, cat_code, cat_brand: string;
  p: integer;
...
begin
  ...
  Main.sIDs := '';
  //удалить _new
  Data.RemoveTableFromBase(Data.SkladQuants.TableName + '_New');
  ...
  try //finally
    UpdateTable.DatabaseName := fLocalDatabase.DatabaseName;
    UpdateTable.TableName := Data.SkladQuants.TableName + '_New';
    Data.CopyMaketTable(UpdateTable, Data.SkladQuants);
    TestTable(UpdateTable);
        ...
        begin
          ...
          if Data.memBrand.FindKey([cat_brand]) then
            if Data.Nom.Locate('Code2;Brand_id',
                                VarArrayOf([cat_code,Data.memBrand.FieldByName('Brand_id').AsInteger]), []) then
            begin
              if UpdateTable.Locate('Cat_ID', Data.Nom.FieldByName('Cat_id').AsInteger,[]) then
              begin
                dir := IntToStr(Prev_Cat_ID) + cb;
                Continue;
              end;

              UpdateTable.Append;
              UpdateTable.FieldByName('CAT_ID').AsInteger := Data.Nom.FieldByName('Cat_id').AsInteger;
              UpdateTable.FieldByName('PRICE').AsString := aReader.Fields[1];
              //UpdateTable.FieldByName('SALE').AsString := aReader.Fields[2];
//              for i:=2 to cQuantBasesCount do

              UpdateTable.FieldByName('Q2').AsString := aReader.Fields[3];
              UpdateTable.FieldByName('L2').AsString := aReader.Fields[4];
              UpdateTable.FieldByName('Q3').AsString := aReader.Fields[5];
              UpdateTable.FieldByName('L3').AsString := aReader.Fields[6];
              UpdateTable.FieldByName('Q4').AsString := aReader.Fields[7];
              UpdateTable.FieldByName('L4').AsString := aReader.Fields[8];
              UpdateTable.FieldByName('Q5').AsString := aReader.Fields[9];
              UpdateTable.FieldByName('L5').AsString := aReader.Fields[10];

              UpdateTable.FieldByName('Q6').AsString := aReader.Fields[11];
              UpdateTable.FieldByName('L6').AsString := aReader.Fields[12];
              UpdateTable.FieldByName('Q7').AsString := aReader.Fields[13];
              UpdateTable.FieldByName('L7').AsString := aReader.Fields[14];
              UpdateTable.FieldByName('Q8').AsString := aReader.Fields[15];
              UpdateTable.FieldByName('L8').AsString := aReader.Fields[16];
              UpdateTable.FieldByName('Q9').AsString := aReader.Fields[17];
              UpdateTable.FieldByName('L9').AsString := aReader.Fields[18];

              UpdateTable.FieldByName('Q10').AsString := aReader.Fields[19];
              UpdateTable.FieldByName('L10').AsString := aReader.Fields[20];
              UpdateTable.FieldByName('Q11').AsString := aReader.Fields[21];
              UpdateTable.FieldByName('L11').AsString := aReader.Fields[22];
              UpdateTable.FieldByName('Q12').AsString := aReader.Fields[23];
              UpdateTable.FieldByName('L12').AsString := aReader.Fields[24];
              UpdateTable.FieldByName('Q13').AsString := aReader.Fields[25];
              UpdateTable.FieldByName('L13').AsString := aReader.Fields[26];

              UpdateTable.FieldByName('Q14').AsString := aReader.Fields[27];
              UpdateTable.FieldByName('L14').AsString := aReader.Fields[28];
              UpdateTable.FieldByName('Q16').AsString := aReader.Fields[29];
              UpdateTable.FieldByName('L16').AsString := aReader.Fields[30];
              UpdateTable.FieldByName('Q17').AsString := aReader.Fields[31];
              UpdateTable.FieldByName('L17').AsString := aReader.Fields[32];
              UpdateTable.FieldByName('Q18').AsString := aReader.Fields[33];
              UpdateTable.FieldByName('L18').AsString := aReader.Fields[34];

              UpdateTable.FieldByName('Q20').AsString := aReader.Fields[35];
              UpdateTable.FieldByName('L20').AsString := aReader.Fields[36];
              UpdateTable.FieldByName('Q21').AsString := aReader.Fields[37];
              UpdateTable.FieldByName('L21').AsString := aReader.Fields[38];
              UpdateTable.FieldByName('Q22').AsString := aReader.Fields[39];
              UpdateTable.FieldByName('L22').AsString := aReader.Fields[40];

              UpdateTable.Post;
              Prev_Cat_ID := Data.Nom.FieldByName('Cat_id').AsInteger;
{              if (aReader.Fields[1] <> '') and (aReader.Fields[1] <> '0') then
              begin
                if Main.sIDs <> '' then
                  Main.sIDs := Main.sIDs + ',' + Data.Nom.FieldByName('Cat_id').AsString
...
begin
  ...
  try
    ...
    if aPackage.PackageType in [wupDataDiscret, wupPictsDiscret, wupTyp] then
      aPass := UPD_PWD + '-' + Copy(aPackage.ZipFile, Length(aPackage.ZipFile) - 11, 8) // вырезаем версию: data_d[110624.1].zip
...
//**************
    ...
    case aPackage.PackageType of
      ...
      wupQuantsStock: Result := DoInstall_QuantsStockNew;
                     {begin
                        if SameText(aPackage.fPackageTypeCode,'quantsxx') then
                          Result := DoInstall_QuantsStock(aQuantsNum)
                        else
                          Result := DoInstall_QuantsStock(aPackage.fPackageTypeCode)
                      end;}
      ...
      wupUnknown:     raiseUpdateErr('Неизвестный тип пакета обновления');
    else
      Result := False;
    end;
